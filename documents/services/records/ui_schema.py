...
class VersionsUISchema(DictOnlySchema):
    """Versions schema."""

    is_latest = ma_fields.Boolean()
    is_latest_draft = ma_fields.Boolean()
    index = ma_fields.Integer()
...
class DocumentsUISchema(UIRequestsSerializationMixin, NRDocumentRecordUISchema):
    ...
    versions = ma_fields.Nested(VersionsUISchema, dump_only=True)
