...
def create_app_blueprint(app):
    blueprint = Blueprint(
        "documents_file_app", __name__, url_prefix="/docs/<pid_value>"
    )
...
def init_addons_documents_file_requests(state):
    app = state.app
...
def init_addons_documents_file_published_service(state):
    """Init app."""
    app = state.app
    ext = app.extensions["documents"]
