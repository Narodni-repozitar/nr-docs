...
def create_app_blueprint(app):
    blueprint = Blueprint("nr_documents_app", __name__, url_prefix="/nr-documents/")
...
def init_addons_nr_documents_requests(state):
    ...
    permissions = getattr(config, "REQUESTS_PERMISSION_POLICY", None)
    if permissions:
        app.config["REQUESTS_PERMISSION_POLICY"] = permissions
