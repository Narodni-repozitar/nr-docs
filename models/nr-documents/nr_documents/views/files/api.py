...
def create_api_blueprint(app):
    """Create NrDocumentsFile blueprint."""
    blueprint = app.extensions["nr_documents"].resource_files.as_blueprint()
    ...
    funcs = [
        ...
        if k.startswith("init_addons_nr_documents_file") and callable(v)
    ]
    for func in funcs:
...
def init_addons_nr_documents_file_requests(state):
    app = state.app
...
def init_addons_nr_documents_file_published_service(state):
    """Init app."""
    app = state.app
    ext = app.extensions["nr_documents"]
