...
OUT=/tmp/deployment_logs/log-nr.md

echo "# Log between $first_deployment_tag and $second_deployment_tag" > $OUT
echo "" >> $OUT

# get requirements.txt from the first deployment
git show $first_deployment_tag:requirements.txt | grep 'nr-' | sed 's/nr-metadata/nr-model/' | sort > /tmp/deployment_logs/first_requirements.txt
git show $second_deployment_tag:requirements.txt  | grep 'nr-' | sed 's/nr-metadata/nr-model/' | sort > /tmp/deployment_logs/second_requirements.txt
...
cat /tmp/deployment_logs/first_requirements.txt | while read line; do
    ...
    if [ -z "$second_version" ]; then
        ...
        package_url="https://api.github.com/repos/narodni-repozitar/$package/compare/$version...$second_version"
        echo $package_url
