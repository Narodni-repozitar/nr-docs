...
.venv/bin/invenio oarepo oai harvester add nusl-offline-files --name "Offline NUSL harvester with files" \
            --url https://s3.cl4.du.cesnet.cz --set global --prefix marcxml \
            --loader 's3{bucket=nr-repo-docs-harvest,harvest_name=nusl-harvest-02}' \
            --transformer marcxml --transformer nusl \
            --writer 'composite{service=published_documents}'

invenio oarepo oai harvester run nusl-offline-files
