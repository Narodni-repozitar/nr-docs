...
def transform_subjects(rec, lang="en"):
    if rec.get("keyword", ""):
        return [{"subject": [{"lang": lang, "value": rec.get("keyword")}]}]
...
def transform_additional_titles(rec, lang="en"):
    return [
        {
            "title": {"lang": lang, "value": title.get("description", "")},
            "titleType": title.get("type", {}).get("id", ""),
        }
        for title in rec.get("additional_descriptions", [])
        if title.get("type", {}).get("id", "") != "notes"
    ]
...
def transform_relation_type(zenodo_relation_type):
    ...
    return {"id": zenodo_relation_type, "title": {"en": en, "cs": cs}}
...
def transform_related_items(rec, lang="en"):
    ...
    for related_identifier in related_identifiers:
        related_item = {
            ...
            "itemRelationType": transform_relation_type(
                related_identifier.get("relation")
            ),
        }
        ...
        if related_identifier.get("scheme", "").upper() == "URL" or (
            ...
            related_item["itemPIDs"] = [{"identifier": identifier, "scheme": scheme}]
...
def get_scheme_and_identifier(identifier):
    ...
    if (
        "scheme" in identifier and identifier.get("scheme") != "pmid"
    ):  # not supporing pmid scheme
...
def transform_rights_to_ours(zenodo_rights):
    rights_mapping = {
        "cc-by-4.0": {
            "id": "4-BY",
            "title": {"en": "Creative Commons Attribution 4.0 International License"},
        },
        "cc-by-nc-nd-4.0": {
            "id": "4-BY-NC-ND",
            "title": {
        ...
        "cc-by-nc-4.0": {
            "id": "4-BY-NC",
            "title": {
                "en": "Creative Commons Attribution-NonCommercial 4.0 International License"
            },
        },
        ...
        "cc-by-sa-4.0": {
            "id": "4-BY-SA",
            "title": {
                "en": "Creative Commons Attribution-ShareAlike 4.0 International License"
            },
        },
        "cc-by-nc-nd-1.0": {
            "id": "1-BY-ND-NC",
            "title": {
                "en": "Creative Commons Attribution-NoDerivs-NonCommercial 1.0 Generic License"
            },
        },
    }
...
def transform_person_or_org(creatibutor):
    ...
    if creatibutor.get("orcid"):
        res["identifiers"] = [
            {"identifier": creatibutor.pop("orcid"), "scheme": "orcid"}
        ]
    ...
    if "," in name:
        ...
        res.update(
            {"given_name": given.strip(), "family_name": family.strip(), "name": name}
        )
    else:
...
def transform_creatibutor(creatibutor):
    res = {"person_or_org": transform_person_or_org(creatibutor)}
...
def transform_resource_type(rec, lang="en"):
    ...
    if (
        resource_type.get("type", {}) == "other"
        or rec.get("publication_type", "") == "other"
    ):
        return {"id": "other", "title": {"en": "Other specialized materials"}}

    if (
        resource_type.get("type", {}) == "publication"
        or rec.get("publication_type", "") == "article"
    ):
